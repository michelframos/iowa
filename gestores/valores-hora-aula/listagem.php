    ...
    <div class="table-responsive">
        <table class="table pmd-table table-hover">
            ...
            <tr>
                ...
                <th width="100">Aplicar Categoria</th>
            ...
            if(isset($_POST['valor'])):
                ...
                $registros = Valores_Hora_Aula::all(array('conditions' => array('nome like ?', '%'.$valor.'%'),'order' => 'nome asc'));
                if(!empty($registros)):
                    foreach($registros as $registro):
                        echo '<tr>';
                        echo '<td data-title="Data Cadastro">'.$registro->data_criacao->format("d/m/Y").'</td>';
                        echo '<td data-title="Empresa/Unidade">'.$registro->nome.'</td>';
                        echo '<td class="texto-centro" data-title="Valor Hora/Aula">R$ '.number_format($registro->valor, 2, ",", ".").'</td>';

                        echo '<td data-title="Aplicar Categoria">';
                        echo '<div class="pmd-switch">';
                        echo '<label>';
                        echo $registro->aplicar_categoria == 's' ? '<input type="checkbox" checked>' : '<input type="checkbox">';
                        echo '<span class="pmd-switch-label aplicar-categoria" registro="'.$registro->id.'"></span>';
                        echo ' </label>';
                        echo '</div>';
                        echo '</td>';
