<?php
    ...
    verificaPermissao(idUsuario(), 'Alunos', 'c', 'index');
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Confirma a exclusão deste Aluno? Esta ação é irreversível! </p>
            </div>
...
<!-- Start Content -->
    ...
    <section class="pmd-card pmd-z-depth padding-10">
        ...
        <div class="espaco20"></div>
        <a href="javascript:void(0);" class="btn btn-danger pmd-btn-raised" id="bt-novo"> Novo Aluno</a>
        <div class="espaco20"></div>
        ...
        <form action="" name="formPesquisa" id="formPesquisa" method="post">
            ...
            <div class="form-group pmd-textfield pmd-textfield-floating-label pmd-textfield-floating-label-completed coluna-3">
                <label>Origem</label>
                <select name="origem" id="origem" class="select-simple form-control pmd-select2 select2-hidden-accessible" tabindex="-1" aria-hidden="true">
                    <option value="%">Todas</option>
                    <?php
                    $origens = Origem_Aluno::all(array('conditions' => array('status = ? or id = ?', 'a', $registro->id_origem), 'order' => 'origem asc'));
                    if(!empty($origens)):
                        foreach($origens as $origem):
                            echo '<option value="'.$origem->id.'">'.$origem->origem.'</option>';
                        endforeach;
                    endif;
                    ?>
                </select>
