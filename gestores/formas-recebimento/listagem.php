    ...
    <div class="table-responsive">
        <table class="table pmd-table table-hover">
            ...
            if(isset($_POST['valor_pesquisa'])):
                ...
                $registros = Formas_Pagamento::all(array('conditions' => array('forma_pagamento like ?', '%'.$valor_pesquisa.'%'),'order' => 'forma_pagamento asc'));
                if(!empty($registros)):
                    foreach($registros as $registro):
                        echo '<tr>';
                        echo '<td data-title="Data Cadastro">'.$registro->data_criacao->format("d/m/Y").'</td>';
                        echo '<td data-title="Descrição">'.$registro->forma_pagamento.'</td>';
                        echo '<td data-title="Taxa de Desconto" class="texto-center">'.number_format($registro->taxa, 5,',', '').' %</td>';
