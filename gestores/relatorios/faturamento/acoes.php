...
if($dados['acao'] == 'gerar-relatorio'):
    ...
    if($a_receber == 'n'):
        ...
        if(!empty($data_inicial_pagamento)):
            $sql_data_vencimento = ' and data_vencimento between "'.$data_inicial_pagamento.'" and "'.$data_final_pagamento.'" ';
        else:
            $sql_data_vencimento = '';
        endif;
    else:
    ...
    if($unidade != 9):
        $parcelas = V_Parcelas2::all(array('conditions' => array('COALESCE(id_unidade, "") like ? and COALESCE(id_turma, "") like ? and COALESCE(id_empresa, "") like ? and (COALESCE(renegociada, "") = "n" or COALESCE(renegociada, "") = "") and COALESCE(id_forma_pagamento, "") like ? '.$situacao_parcela.$parcelas_canceladas.$sql_data_pagamento.$sql_data_vencimento.$vencidas, $unidade, $id_turma, $id_empresa, $forma_pagamento ),  'order' => 'COALESCE(nome_empresa, ""), COALESCE(nome, "") asc'));
    elseif($unidade == 9):
        $parcelas_alunos = V_Parcelas2::all(array('conditions' => array('COALESCE(id_unidade, "") like ? and COALESCE(id_turma, "") like ? and id_empresa like ? and (COALESCE(renegociada, "") = "n" or COALESCE(renegociada, "") = "") and COALESCE(id_forma_pagamento, "") like ? and pagante = ?'.$situacao_parcela.$parcelas_canceladas.$sql_data_pagamento.$sql_data_vencimento.$vencidas, $unidade, $id_turma, $id_empresa, $forma_pagamento, 'aluno' ),  'order' => 'COALESCE(nome_empresa, ""), COALESCE(nome, "") asc'));
        $parcelas_empresas = V_Parcelas2::all(array('conditions' => array('COALESCE(id_unidade, "") like ? and COALESCE(id_turma, "") like ? and COALESCE(id_empresa, "") like ? and (COALESCE(renegociada, "") = "n" or COALESCE(renegociada, "") = "") and COALESCE(id_forma_pagamento, "") like ? and pagante = ?'.$situacao_parcela.$parcelas_canceladas.$sql_data_pagamento.$sql_data_vencimento.$vencidas, '%', '%', '%', $forma_pagamento, 'empresa' ),  'order' => 'COALESCE(nome_empresa, ""), COALESCE(nome, "") asc'));
        $parcelas = array_merge($parcelas_alunos, $parcelas_empresas);
    endif;
            ...
            <div class="table-responsive">
                <table class="table pmd-table table-hover">
                    ...
                    <tr>
                        ...
                        <th class="texto-center">Forma Pagamento</th>
                        <th class="texto-center">Vr</th>
                        <th class="texto-center">Vr Pago</th>
                        <th class="texto-center">Vr Pago Com Taxas</th>
                    </tr>
                    </thead>
                    <tbody>
                    ...
                    if(!empty($parcelas)):
                        ...
                        foreach($parcelas as $parcela):
                            ...
                            try{
                                $taxa_forma_pagamento = Formas_Pagamento::find($parcela->id_forma_pagamento);
                                $taxa = $taxa_forma_pagamento->taxa;
                            } catch (Exception $e){
                                $taxa = 0;
                            }
                            ...
                            echo '<tr>';
                                ...
                                echo !empty($taxa_forma_pagamento) ? '<td class="text-center">'.$taxa_forma_pagamento->forma_pagamento.'</td>' : '<td></td>';


                                echo '<td class="text-right"> '.number_format($parcela->total, 2, ',','.').'</td>';
                                echo '<td class="text-right"> '.number_format($parcela->valor_pago, 2, ',','.').'</td>';
                                echo '<td class="text-right"> '.number_format($parcela->valor_pago-($parcela->valor_pago*$taxa), 2, ',','.').'</td>';
                            ...
                            $total_pago_com_taxas += $parcela->valor_pago+($parcela->valor_pago*$taxa);

                        endforeach;

                        echo '<tr>';
                        echo '<td colspan="2" class="bold size-1-5" id="total-registros">Total de Registro: '.$numero_registros.'</td>';
                        echo '<td colspan="2" class="bold size-1-5" id="total-valor">Total: R$ '.number_format($total, 2, ',', '.').'</td>';
                        echo '<td colspan="2" class="bold size-1-5" id="total-valor">Total Pago: R$ '.number_format($total_pago, 2, ',', '.').'</td>';
                        echo '<td colspan="3" class="bold size-1-5" id="total-valor">Faturamento: R$ '.number_format($total_pago, 2, ',', '.').'</td>';
                        echo '</tr>';
