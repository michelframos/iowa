...
if($dados['acao'] == 'gerar-relatorio'):
    ...
    $turmas = Turmas::find_by_sql("
        ...
        where 
        turmas.id_unidade like '".$id_unidade."' 
        ...
        foreach($turmas as $turma):
            ...
            ?>
                ...
                foreach($alunos as $aluno):
                    ...
                    $matricula = Matriculas::find_by_id_aluno_and_id_turma($aluno->id_aluno, $turma->id_turma);
                    ...
                    $total_faltas = 0;
                    if(!empty($total_aulas)):
                        foreach($total_aulas as $aula):
                            if(Aulas_Alunos::find_by_id_aula_and_id_aluno_and_presente($aula->id, $aluno->id_aluno, 'n')):
                                $total_faltas++;
                            endif;
                        endforeach;
                    ...
                    if($considerar_abono == 'n'):
                        ...
                        if($numero_aulas > 0):
                            ...
                            $aproveitamento = ((($numero_aulas-$total_faltas)+(int)$abonos)/$numero_aulas)*100;
                        endif;
                    endif;
                    ...
                    echo ($considerar_abono == 's' && $matricula->responsavel_pedagogico == 2) ? '<td class="texto-centro" width="5%">'.($numero_faltas-$abonos).'</td>' : '<td class="texto-centro" width="5%">'.$numero_faltas.'</td>' ;
                        ...
                        if(!empty($aulas)):
                            foreach($aulas as $aula):
                                if($considerar_abono == 'n' && $matricula->responsavel_pedagogico == 2):
                                    ...
                                    //if(Aulas_Alunos::find_by_id_aula_and_id_aluno_and_presente_and_abonada($aula->id, $aluno->id_aluno, 'n', 'n')):
                                    if(Aulas_Alunos::all(array('conditions' => array('id_aula = ? and id_aluno = ? and presente = ? and coalesce(abonada, "n") = ?', $aula->id, $aluno->id_aluno, 'n', 'n')))):
                                        echo $aula->data->format('d/m/Y').' - ';
                                    endif;
                                endif;
                            endforeach;
