...
if($dados['acao'] == 'salvar'):
    ...
    $verifica_datas = Aulas_Help::find_by_id_colega_and_data($dados['id_colega'], $data_inicio);
    ...
    if(!empty($dias_selecionados)):
        ...
        foreach($dias_selecionados as $dias_selecionado):
            ...
            if($j < 1):
                $verifica_datas = Aulas_Help::find_by_id_colega_and_data($dados['id_colega'], $data->format('Y-m-d'));
                if(!empty($verifica_datas)):
                    echo json_encode(array('status' => 'erro_data', 'data' => $data->format('d/m/Y')));
                    exit();
                endif;
...
if($dados['acao'] == 'aprovar'):
    ...
    if(Aulas_Help::find_by_id_colega_and_data($registro->id_colega, $verifica_data->format('Y-m-d'))):
