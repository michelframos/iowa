    ...
    <div class="table-responsive">
        <table class="table pmd-table table-hover">
            ...
            if(isset($_POST['valor_pesquisa'])):
                ...
                $registros = Categorias_Lancamentos::all(array('conditions' => array('categoria like ?', '%'.$valor_pesquisa.'%'),'order' => 'categoria asc'));
                if(!empty($registros)):
                    foreach($registros as $registro):
                        echo '<tr>';
                        echo '<td data-title="Data Cadastro">'.$registro->data_criacao->format("d/m/Y").'</td>';
                        echo '<td data-title="Origem">'.$registro->categoria.'</td>';
