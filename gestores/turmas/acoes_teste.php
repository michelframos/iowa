...
if($dados['acao'] == 'salvar'):
    ...
    $aulas_turma = Aulas_Turmas::all(array('conditions' => array('id_turma = ? and id_situacao_aula <> ?', $registro->id, 0)));
    if(!empty($aulas_turma)):
        $aulas_dadas = 0;
        foreach($aulas_turma as $aula_turma):
            $data_inicial = $aula_turma->data;
    ...
    $produto_antigo = Nomes_Produtos::find($registro->id_produto);
    $horas_dadas = $produto_antigo->horas_semanais*$aulas_dadas;
    ...
    echo $produto->nome_material.'<br>';

    if(empty($aulas_dadas) || $aulas_dadas == 0):
        $qtd = ceil($produto->numero_aulas/$dias_semana);
    else:
        $novo_numero_aulas = ceil(($produto->horas_estagio-$horas_dadas)/$produto->horas_semanais);
        echo $novo_numero_aulas.'<br>';
